...
void caesar_cipher(unsigned char plaintext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 //     tests it means you did not test carefully enough.
  ...
  while (plaintext[i] != '\0'){
    ...
    // printf("i = %u, passPic[%u][%u] = %u\n", i, y, x, passPic[y][x]);

    if (plaintext[i] + passPic[y][x] == '\0'){
      // printf("BONEZONE\n");
      plaintext[i] = plaintext[i] + passPic[y][x] + 1;  
    }else{
      // Encrypting, so add. //
      plaintext[i] = plaintext[i] + passPic[y][x];
      i++;
      x++;
    }
    ...
    if (x == MAXPIC){
      // printf("Entered the X = 4 ZONE\n");
      x = 0;
      y++;
    } 

    if (y == MAXPIC){
      // printf("Entered the Y = 4 ZONE\n");
      y = 0;
    }
  } 
}
...
void caesar_decipher(unsigned char cryptotext[MAX_STR_LEN], unsigned char passPic[50][50])
{
 ...
 // TO DO: Implement this function
  ...
  while (cryptotext[i] != '\0'){
    ...
    if (cryptotext[i] - passPic[y][x] == '\0'){
      // printf("BONEZONE\n");
      cryptotext[i] = cryptotext[i] - passPic[y][x] - 1;
    }else{
      // Decrypting, so subtract. //
      cryptotext[i] = cryptotext[i] - passPic[y][x];
      i++;
      x++;
    }
