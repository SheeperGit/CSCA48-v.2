...
// Helper Function: Does the same thing as ingredient_index(), but w/ recursion.
//                  I don't know if I really need to use recursion on every function however...
int ingredient_index_recursive(char source_ingredient[MAX_STR_LEN], int i){
  // Match Found! //
  if (strcmp(ingredients[i], source_ingredient) == 0) return i;

  // Hit the end of the ingredients list. No ingredient found... //
  if (i == MAT_SIZE) return -1;

  // Go the next entry in the ingredients list! //
...
int ingredient_index(char source_ingredient[MAX_STR_LEN]){
    ...
    return ingredient_index_recursive(source_ingredient, 0);

    // Below lies the iterative solution //
...
void related_ingredients(char source_ingredient[MAX_STR_LEN]){
    ...
    for (int i = 0; i < MAT_SIZE; i++){
      if (AdjMat[i][src_index] > 0) printf("%s\n", ingredients[i]);
    }
}
